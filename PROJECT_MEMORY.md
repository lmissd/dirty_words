...
用户进一步明确，唤醒后的正式录音不应再固定录满 7 秒。当前录音链路应优先复用 `post_wake_speech` 捕获到的预录音内容，并从该时刻继续录制，直到检测到一段连续静音后自动收尾；`7` 秒只保留为安全上限 `recording.max_duration_seconds`，避免异常情况下无限等待。

项目协作中，测试临时产物要及时清理，避免目录混乱。尤其是 `recordings/`、`exports/`、`tmp/`、训练采样中途产生的试听或临时文件，验证完成后应删除或保持在 `.gitignore` 范围内，不作为正式资产提交。
